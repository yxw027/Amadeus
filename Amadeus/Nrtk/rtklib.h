...
#define OUTPUT_RNXOBS
#define OUTPUT_RNXNAV
#define OUTPUT_RES
...
#define EXPORT
	...
	struct nav_all
	{
		...
		int    flag ;     //星历存在标石，1，ok，0.不存在
		int    prn;       //卫星prn号码
		double a[31];     //星历参数
		int    sys;       //系统标石 GPS 1，GLONASS 2，
...
#define WAVELENGTH_L1   0.1902936727984
#define WAVELENGTH_L2   0.2442102134246
...
// 采用二进制标识
// 00000001 [NULL                                   GPS] =1
// 00000010 [NULL                           GLONASS    ] =2
// 00000100 [                           BDS            ] =4
// 00000101 [NULL                       BDS         GPS] =5
// ......
// 10000000 [NULL LEO SBAS QZSS GALIELO BDS GLONASS GPS] =127
...
#define SYS_GC      0x05                /* navigation system: GPS + BDS */
...
#ifndef NEXOBS
#define NEXOBS      2       //原来值为0，turui2015.11.3            /* number of extended obs codes */
#endif
...
#define MINSINANCMP  141
#define MAXSINANCMP  177              /* sinan special prn*/
...
#define MAXDTOE_CMP 3700.0              /* max time difference to BeiDou Toe (s) */
...
#define EARTH_RADIUS 6378136.0               /* earth raedius */
...
#define P2_66       1.355252715606880E-20 /* 2^-66 */ //duyuan--2015.11.06
#define P2_add11    2048                  /* 2^11  */ //duyuan--2015.11.06

// add by hjq 2016-08-30 for sin(x)=[-1,1] as well as cos(x)
#define ASIN_BOUND(x) (fabs(x-1.0)<1.0E-009?1.0:(fabs(x+1.0)<1.0E-009?-1.0:(fabs(x)<=1.0?x:0)))
...
#define FILEPATHSEP '/'
	...
	typedef struct {        /* RTCM control struct type */
		...
		int satprn[MAXSAT];  //卫星PRN号码---add  by -------------------------------
		int type;    //add by ---------2012.7.9-----------文件类型obs,nav,correction
		int navsum[MAXSAT];  //add by ---------2012.7.9-----------总的卫星个数及卫星号码
		int obssum[MAXSAT];  //add by ---------2012.7.9-----------
		int corsum[MAXSAT];  //add by ---------2012.7.9-----------
		gtime_t correctime;  //add by ---------2012.7.11----------
		int obssat;          //add  by ---------2012.7.11---------
	} rtcm_t;
	...
	typedef struct {        /* RTK control/result type */
		...
		int Refsat;         //参考卫星，涂锐-------------------------------------
		double *ambfixL1;    //固定后的模糊度L1-----------------
		double *ambfixL2;   //固定后的模糊度L2------------------
		double *ambfloatL1;    //固定后的模糊度L1-----------------
		double *ambfloatL2;   //固定后的模糊度L2------------------
		double *ambfixepochL1;   //固定后的系统的模糊度L1--2015.10.17--
		double *ambfixepochL2;   //固定后的系统的模糊度L2--2015.10.17--
		int ambfloatflagL1[32];     //固定前的本历元的模糊度L2--2015.10.17--
		int ambfloatflagL2[32];     //固定前的本历元的模糊度L2--2015.10.17--
		int ambfixflagL1[32];   //固定后的本历元的模糊度L1--2015.10.17--
		int ambfixflagL2[32];    //固定后的本历元的模糊度L1--2015.10.17--
		int ambfixsysflagL1[32];   //固定后的系统的模糊度L1--2015.10.17--
		int ambfixsysflagL2[32];    //固定后的系统的模糊度L1--2015.10.17--

		int ambflag;       //是否固定标志-------------------
		int comsatnum;     //共同卫星个数-----------
		int satNo[MAXSAT];    //共同的卫星号码
		double *ionfloat;   //dianliceng
		double tropfloat[2];    //duiliuceng
		double *ionfix;   //dianliceng
		double *tropmapR;   
		double *tropmapU;
		double *ionmapR;
		double *ionmapU;   
		double tropfix[2];    //duiliuceng
	} rtk_t;
	...
	extern int  satno_sn (int sys, int prn);
	...
	extern void matmul (const char *tr100, int n, int k, int m, double alpha100, 
		const double *A100, const double *B100, double beta100, double *C100);

	void matmulnew (char *tr100, int n, int k, int m, double alpha100, 
		double *A100,  double *B100, double beta100, double *C100);
	...
	extern int     is_tint(gtime_t time, double tint);
	...
	extern void changenavNo(nav_t *nav);
	...
	extern int outrnxobsh(FILE *fp, const rnxopt_t *opt, const nav_t *nav);   //-------2015/7/27
	...
	extern int decode_D1eph(const unsigned char *buff ,eph_t *eph);
	extern int decode_D2eph(const unsigned char *buff ,eph_t *eph);
	...
	extern int input_sinan (rtcm_t *rtcm,rtcm_t *out, unsigned char data);
