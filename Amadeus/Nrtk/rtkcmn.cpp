...
const prcopt_t prcopt_default={ /* defaults processing options */
    ...
    5,0,10,                   /* maxout,minlock,minfix,armaxiter */
...
extern int satno_sn(int sys, int prn)
{
    ...
    switch (sys) {
        ...
        case SYS_QZS:
            ...
            return NSATGPS+NSATGLO+NSATGAL+prn-MINPRNQZS+1;
        case SYS_CMP:
            if (prn<MINSINANCMP||MAXSINANCMP<prn) return 0;
            return NSATGPS+NSATGLO+NSATGAL+NSATQZS+prn-MINSINANCMP+1;
        case SYS_LEO:
            if (prn<MINPRNLEO||MAXPRNLEO<prn) return 0;
            return NSATGPS+NSATGLO+NSATGAL+NSATQZS+NSATCMP+prn-MINPRNLEO+1;
        case SYS_SBS:
...
void matmulnew(char *tr100, int n, int k, int m, double alpha100,
	double *A100, double *B100, double beta100, double *C00)
{
  ;
}
...
{
    int lda=tr100[0]=='T'?m:n,ldb=tr100[1]=='T'?k:m;
    
    dgemm_((char *)tr100,(char *)tr100+1,&n,&k,&m,&alpha100,(double *)A100,&lda,(double *)B100,
           &ldb,&beta100,C100,&n);
}
/* inverse of matrix -----------------------------------------------------------
...
{
    double d;
    int i,j,x,f=tr100[0]=='N'?(tr100[1]=='N'?1:2):(tr100[1]=='N'?3:4);
    ...
    for (i=0;i<n;i++) for (j=0;j<k;j++) 
	{
        ...
        switch (f) 
		{
            case 1: for (x=0;x<m;x++) d+=A100[i+x*n]*B100[x+j*m]; break;
            case 2: for (x=0;x<m;x++) d+=A100[i+x*n]*B100[j+x*k]; break;
            case 3: for (x=0;x<m;x++) d+=A100[x+i*m]*B100[x+j*m]; break;
            case 4: for (x=0;x<m;x++) d+=A100[x+i*m]*B100[j+x*k]; break;
        }
        if (beta100==0.0) C100[i+j*n]=alpha100*d;
		else C100[i+j*n]=alpha100*d+beta100*C100[i+j*n];
    }
}
/* LU decomposition ----------------------------------------------------------*/
...
extern void changenavNo(nav_t *nav)
{
	...
	for(int i=0;i<n;i++)
	{
		nav->eph[i].sat=nav->eph[i].sat-(NSATGPS+NSATGLO+NSATGAL+NSATQZS-MINPRNCMP+1);
		int tu=nav->eph[i].sat;
	}
}
